...
public class BeerOrderControllerTest extends BaseIT {
    ...
    @BeforeEach
    void setUp() {
        mockMvc = MockMvcBuilders //this setUp is overriding the BaseIT, that's why the mockMvc is builded here
    ...
    @WithUserDetails("spring") //With this, we don't need HttpBasic
    @Test
    ...
    @Transactional //getBeerOrders() has a lazily initialize
    @Test
    ...
    private BeerOrderDto buildOrderDto(Customer customer, UUID beerId) {
        List<BeerOrderLineDto> beerOrderLineDtos = Arrays.asList(BeerOrderLineDto.builder()
                .id(UUID.randomUUID())
                .beerId(beerId)
                .orderQuantity(5)
                .build());
